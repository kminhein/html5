        ...
        applyAction: function (img, dataImg, actionName, options) {
            ...
            if (w == 0 || h == 0) {
                if (img.parentNode == null) {
                    ...
                    if (Pixastic.debug)
                        writeDebug("Image has 0 width and/or height.");
                    return;
                }
            }
            ...
            if (!validAction) {
                if (Pixastic.debug)
                    writeDebug("Invalid action \"" + actionName + "\". Maybe file not included?");
                return false;
            }
            if (!Pixastic.Actions[actionName].checkSupport()) {
                if (Pixastic.debug)
                    writeDebug("Action \"" + actionName + "\" not supported by this browser.");
                return false;
            }
...
Pixastic.Actions.crop = {
    process: function (params) {
        if (Pixastic.Client.hasCanvas()) {
            ...
            if (left < 0)
                left = 0;
            if (left > params.width - 1)
                left = params.width - 1;

            if (top < 0)
                top = 0;
            if (top > params.height - 1)
                top = params.height - 1;
...
Pixastic.Actions.glow = {
    process: function (params) {
        ...
        if (Pixastic.Client.hasCanvasImageData()) {
            ...
            while (p--) {
                if ((data[pix -= 4] += amount * blurData[pix]) > 255)
                    data[pix] = 255;
                if ((data[pix1 -= 4] += amount * blurData[pix1]) > 255)
                    data[pix1] = 255;
                if ((data[pix2 -= 4] += amount * blurData[pix2]) > 255)
                    data[pix2] = 255;
            }
