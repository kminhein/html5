...
//echo "<pre>"; print_r($this->data['priceList']->result()); die;
$priceListVal= $this->data['priceList']->result();
...
<div id="content">
		<div class="grid_container">
			<?php 
				...
				echo form_open('admin/pricing/change_pricing_status_global',$attributes) 
			?>
			<div class="grid_12">
				<div class="widget_wrap">
					<div class="widget_top">
						...
						<?php if ($allPrev == '1' || in_array('2', $pricerange)){?>
						...
						if ($allPrev == '1' || in_array('3', $pricerange)){
						?>
							<div class="btn_30_light" style="height: 29px;">
					...
					<div class="widget_content">
						...
						<tbody>
						<?php 
						if (count($priceListVal) > 0){
							foreach ($priceListVal as $row){
							 if($row->id!=''){
						?>
						<tr>
...
<!-- 							<td class="center">
 ...
 -->							
							<td class="center">
							<?php 
							if ($allPrev == '1' || in_array('2', $pricerange)){
								$mode = ($row->status == 'Active')?'0':'1';
								if ($mode == '0'){
							?>
								<a title="Click to inactive" class="tip_top" href="javascript:confirm_status('admin/pricing/change_pricing_status/<?php echo $mode;?>/<?php echo $row->id;?>');"><span class="badge_style b_done"><?php echo $row->status;?></span></a>
							<?php
								}else {	
							?>
								<a title="Click to active" class="tip_top" href="javascript:confirm_status('admin/pricing/change_pricing_status/<?php echo $mode;?>/<?php echo $row->id;?>')"><span class="badge_style"><?php echo $row->status;?></span></a>
							<?php 
								}
							}else {
							?>
							...
							<td class="center">
							<?php if ($allPrev == '1' || in_array('2', $pricerange)){?>
								<span><a class="action-icons c-edit" href="admin/pricing/EditviewPricing/<?php echo $row->id;?>" title="Edit">Edit</a></span>
							<?php }?>
							...
							<?php if ($allPrev == '1' || in_array('3', $pricerange)){?>	
								<span><a class="action-icons c-delete" href="javascript:confirm_delete('admin/pricing/delete_pricing_list/<?php echo $row->id;?>')" title="Delete">Delete</a></span>
							<?php }?>
							</td>
						</tr>
						<?php 
