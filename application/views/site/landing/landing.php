...
<?php	$controlview = $layoutList->result_array();
...
?>
	<?php if ($loginCheck != '' ){ ?>
		<div class="container mobile-padding-top desk-padding-top timeline <?php echo $viewhome ; ?>">
	...
	<?php if($flash_data != '') { ?>
		...
		<p class="welcome">
			<strong> <?php foreach($layoutfulllist->result() as $layoutListRow){ 
				if($layoutListRow->place == 'welcome text'){
					echo  $layoutListRow->text;?>
			</strong>
		</p>	
		<p class="tag"> <?php }
			if($layoutListRow->place == 'welcome tag' ){ echo  $layoutListRow->text;?>
		</p>
			...
			<div class="top-menu">
				...
				<?php if ($mainCategories->num_rows()>0){?>
					<li><a href="javascript:everythingView('1');"
						class="current top-menu-btn"><?php if($this->lang->line('everything') != '') { echo stripslashes($this->lang->line('everything')); } else echo "Everything"; ?></a>


						<ul class="everything_list category" id="everythinglist1">


							<li><a data-category="<?php echo base_url();?>" class="active"><?php if($this->lang->line('everything') != '') { echo stripslashes($this->lang->line('everything')); } else echo "Everything"; ?></a>
							</li>
				...
				<div class="viewer">
					<ul>
						...
						<?php if($viewhome == 'classic'){ ?> class="current"
						...
						<li class="normal"><a href="#" <?php if($viewhome == 'normal'){ ?>
							class="current"
							onclic="$('.container.timeline').addClass('classic').find('.wrapper-content').removeAttr('class').addClass('wrapper-content');"
						...
						<?php if($viewhome == 'vertical'){ ?> class="current"
							onclic="$('.container.timeline').addClass('classic').find('.wrapper-content').removeAttr('class').addClass('wrapper-content');"
						...
						<div class="figure-item">
							...
							<a
								...
								class="figure vertical"> <em class="back"></em> <img
									...
									data-height="310"> </span> <span class="figcaption"><?php echo $productArr[$i]->product_name;?>
							</span> </a> <em class="figure-detail"> <?php if (!isset($productArr[$i]->web_link)){?>
								...
								class="username"><em style="padding-left: 0;"><i> <?php if($this->lang->line('user_by') != '') { echo stripslashes($this->lang->line('user_by')); } else echo "by"; ?>
										...
										href="<?php if ($productArr[$i]->user_id != '0'){echo base_url().'user/'.$productArr[$i]->user_name;}else {echo base_url().'user/administrator';}?>"><?php if ($productArr[$i]->user_id != '0'){echo $productArr[$i]->full_name;}else {echo 'administrator';}?>
