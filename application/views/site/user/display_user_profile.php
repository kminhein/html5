    ...
    <div class="wrapper-content right-sidebar">
      <div id="content">
        <div class="wrapper timeline normal">
          <ul class="user-tab">
            <li><a href="user/<?php echo $userProfileDetails->row()->user_name;?>" class="current"><b><?php echo LIKED_BUTTON;?></b> <small><?php echo $userProfileDetails->row()->likes;?></small></a></li>
         ...
         <?php 
         	if ($featureProductDetails->num_rows() == 1){
         		$imgName = 'dummyProductImage.jpg';
         		$imgArr = explode(',', $featureProductDetails->row()->image);
         		if (count($imgArr)>0){
         		...
         		$fancyClass = 'fancy';
          		...
          		if (count($likedProducts)>0 && $likedProducts->num_rows()>0){
          			foreach ($likedProducts->result() as $likeProRow){
          				if ($likeProRow->product_id == $featureProductDetails->row()->seller_product_id){
          ...
          <div class="feature">
          <?php 
          if ($featureProductDetails->mode == 'sell_product'){
          	$turl = 'things/'.$featureProductDetails->row()->id.'/'.url_title($featureProductDetails->row()->product_name);
          }else {
          	$turl = 'user/'.$featureProductDetails->row()->user_name.'/things/'.$featureProductDetails->row()->seller_product_id.'/'.url_title($featureProductDetails->row()->product_name);
          }
          ?>
            <div class="figure-item"> <a href="<?php echo $turl;?>" class="vcard"><img src="images/product/<?php echo $imgName;?>"></a> <a href="<?php echo $turl;?>" class="figure-img"> <span class="figure"><img src="images/product/<?php echo $imgName;?>" style="width:100%;" /><em class="back"></em></span> <span class="figcaption"><?php echo $featureProductDetails->row()->product_name;?></span> </a>
              <h3><?php if($this->lang->line('display_featured_find') != '') { echo stripslashes($this->lang->line('display_featured_find')); } else echo "FEATURED FIND"; ?></h3>
              <em class="figure-detail"> <span class="username"><em><a href="<?php if ($featureProductDetails->row()->username == ''){echo 'user/administrator';}else {echo 'user/'.$featureProductDetails->row()->user_name;}?>"><?php if ($featureProductDetails->row()->user_name == ''){echo 'administrator';}else {echo $featureProductDetails->row()->full_name;}?></a> + <?php echo $featureProductDetails->row()->likes;?></em></span> </em> 
              <a href="#" item_img_url="images/product/<?php echo $imgName;?>" tid="<?php echo $featureProductDetails->row()->seller_product_id;?>" class="button <?php echo $fancyClass;?>" <?php if ($loginCheck==''){?>require_login="true"<?php }?>><span><i></i></span> <?php echo $fancyText;?></a> </div>
          </div>
          <?php }?>
          ...
          ?>
            <li class="big clear">
              <div class="figure-item"> 
              <a href="<?php echo 'user/'.$productLikeDetailsRow->user_name.'/things/'.$productLikeDetailsRow->seller_product_id.'/'.url_title($productLikeDetailsRow->product_name);?>" class="vcard">
              <img src="images/product/<?php echo $imgName;?>">
              </a> 
              <a href="<?php echo 'user/'.$productLikeDetailsRow->user_name.'/things/'.$productLikeDetailsRow->seller_product_id.'/'.url_title($productLikeDetailsRow->product_name);?>" class="figure-img"> 
              <span class="figure grid" style="background-size: cover" data-ori-url="images/product/<?php echo $imgName;?>" data-310-url="images/product/<?php echo $imgName;?>"><em class="back"></em>
              </span> 
              <span class="figure classic"> <em class="back"></em> 
              <img src="images/product/<?php echo $imgName;?>" data-width="640" data-height="640"> 
