...
function writeSitemap($FILE) {
	...
	$LAYOUT->addSuccess('Sitemap successfuly created and saved to <a href="'.$SETTINGS['website'].basename($SETTINGS[PSNG_SITEMAP_FILE]).'" target="_blank">'.basename($SETTINGS[PSNG_SITEMAP_FILE]).'</a>!');
	if (isset($SETTINGS[PSNG_TXTSITEMAP_FILE]) && strlen($SETTINGS[PSNG_TXTSITEMAP_FILE])>0) $LAYOUT->addSuccess('Txt-Sitemap successfuly created and saved to <a href="'.$SETTINGS['website'].basename($SETTINGS[PSNG_TXTSITEMAP_FILE]).'" target="_blank">'.basename($SETTINGS[PSNG_TXTSITEMAP_FILE]).'</a>!');
