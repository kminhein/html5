...
if (!isset($SETTINGS[PSNG_COMPRESS_SITEMAP])) $SETTINGS[PSNG_COMPRESS_SITEMAP] = '';
	...
	<div style=" font-size:18px; text-align:center; width:100%;margin-top:20px;">Loading... Please wait...</div>
	<fieldset style="padding: 10; width:600; border-color:#000099; border-width:2px; border-style:solid; display:none; ">
	<table border="0" cellpadding="5" cellspacing="0" width="495">
	  <tr>
		...
		<td width="396">
			<input class="required" type="Text" name="'.PSNG_PAGEROOT.'" id="ipage_root" align="LEFT" size="50" value="../"/>
	  ...
	  <tr>
		...
		<td>
			<input class="required" type="Text" name="'.PSNG_WEBSITE.'" id="iwebsite" align="LEFT" size="50" value="'.$baseUrl.'"/>
	  ...
	  <tr>
		...
		<td>
			...
			<input type="checkbox" onClick="checkCrawlerEdit()" '.(($SETTINGS[PSNG_SCAN_WEBSITE] != '') ? 'checked':'') .' name="'.PSNG_SCAN_WEBSITE.'" id="iscan_website" align="LEFT" size="50" value="true"/>Scan website - <font size="-1">will scan the website with the build-in crawler engine; might take some time if there are many urls to crawl!
				Optional: crawl only this url: <input type="Text" name="'.PSNG_CRAWLER_URL.'" id="icrawlerurl" align="LEFT" size="50" value="'.$baseUrl.'"/><br/>Will only crawl this url and subdirectories!</font></p>
		</td>
	  </tr>
	...
	</form><script type="text/javascript"> document.forms["setupExpert"].submit();</script>
	</div>';
