...
//alert(viewport);
			...
			.find('ul.figure-list > li > a')
				...
				.click(function(event){
					...
					var $this = $(this), img = $this.attr('href'), size = [0,0], ratio, boundary={}, $zoom, w, h, img1;

					img1 = new Image;
					img1.onload = function(){ $this.data('size', this.width+','+this.height) };
				    try {
...
//					boundary.right  = boundary.left + boundary.width;
	...
	$('.feature,.feature-selected').click(function(){
		...
		if(is_featured){
			url = baseURL+'site/product/delete_featured_find';
//			params.tid = $this.attr('tid');
		}else{
			url = baseURL+'site/product/add_featured_find';
			params.tid = $this.attr('tid');
...
*/		}
	...
	// "I own it" button
	$('.thing-info a.own,.detail_thing_info1 a.own,.detail_thinginfo a.own').click(function(event) {
		...
		if(selected=$this.hasClass(_CLASS_)){
			url = baseURL+'site/product/delete_have_tag';
			params = {thing_id : $this.attr('tid')};
		} else {
			url = baseURL+'site/product/add_have_tag';
			params = {thing_id : $this.attr('tid')};
		}
